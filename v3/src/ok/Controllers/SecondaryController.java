...
public class SecondaryController {
    ...
    void initDeposit(ActionEvent event) {
    	Optional<String> result = utils.showInputDialog("Ａｍｏｕｎｔ　ｔｏ　ｄｅｐｏｓｉｔ：　", this.second_root.getScene(), true).showAndWait();
    	if (result.isPresent() && !result.get().isEmpty()){
    		...
    		case -1: 
    			...
    			break;
    		case 0:
    			utils.showAlertDialog("Ｓｕｍ　ｅｘｃｅｅｄｓ　ｍａｘｉｍｕｍ　ａｌｌｏｗｅｄ　ｄｅｐｏｓｉｔ　ｖａｌｕｅ．", this.second_root.getScene());
    			break;
    		case 1:
    			lbl_currbalance.setText(ah.getAccountByType(0).getBalance() + " " + ah.getAccountByType(0).getCurrency());
    ...
    void initWithdraw(ActionEvent event) {
    	...
    	Optional<String> result = utils.showInputDialog("Ａｍｏｕｎｔ　ｔｏ　withdraw： ", this.second_root.getScene(), true).showAndWait();
    	if (result.isPresent() && !result.get().isEmpty()){
    ...
    void initialize() throws JSONException, IOException {
    	...
    	lbl_currb = this.lbl_currbalance;
    	lbl_currb.setText(ah.getAccountByType(0).getBalance() + " " + ah.getAccountByType(0).getCurrency());
    }
